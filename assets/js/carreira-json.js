...
const renderProfile = ({ profile, contacts }) => {
  ...
  contacts.forEach((contact) => {
    ...
    const link = createElement("a", "text-link social-link", "");
    ...
    link.appendChild(document.createTextNode(contact.label));
...
const renderSkills = ({ skillGroups, otherSkills }) => {
  ...
  skillGroups.forEach((group) => {
    ...
    groupElement.appendChild(
      createElement("h3", "resume-skills-cat font-weight-bold h5", group.title),
    );
...
fetch(DATA_URL)
  ...
  .catch((error) => {
    ...
    const warning = createElement(
      "p",
      "alert alert-warning m-5",
      "Nao foi possivel carregar os dados do JSON. Execute a pagina em um servidor local ou publique no GitHub Pages.",
    );
